 ...
 *              - const polyvecl *s0: polyvecl pointer containing s0 (encoding starting at offset 1)
...
#endif
    // append b into A
    for (i = 0; i < K; ++i) { // TODO unpack directly to A
        A[i].vec[0] = b1.vec[i];
    }
}
 ...
 * Name:        pack_sig
 *
 * Description: Bit-pack signature sig = (c, LB(z1), len(x), x= Enc(HB(z1)), Enc(h)).
 *
 ...
 **************************************************/
int pack_sig(uint8_t sig[CRYPTO_BYTES], const uint8_t c_seed[SEEDBYTES], const polyvecl *lowbits_z1, 
            const polyvecl *highbits_z1, const polyveck *h) {

    uint8_t encoded_h[MAX_ENC_H_BYTES];
    uint8_t encoded_hb_z1[MAX_ENC_HB_Z1_BYTES];
    uint8_t *begin_enc_h, *begin_enc_hb_z1; // encoding populates "encoded_h" from right to left
    ...
    begin_enc_hb_z1 = encode_hb_z1(encoded_hb_z1, &highbits_z1->vec[0].coeffs[0]);
    size_enc_hb_z1 = (uint16_t) (encoded_hb_z1 + MAX_ENC_HB_Z1_BYTES - begin_enc_hb_z1);
    
    begin_enc_h = encode_h(encoded_h, &h->vec[0].coeffs[0]);
    size_enc_h = (uint16_t) (encoded_h + MAX_ENC_H_BYTES - begin_enc_h);

    if(SEEDBYTES + L*N + 2 + size_enc_hb_z1 + size_enc_h > CRYPTO_BYTES) //signature too big
        return 1;

    sig[0] = (uint8_t) (size_enc_hb_z1>>8);
    sig[1] = (uint8_t) (size_enc_hb_z1 & 0xFF);
    sig+= 2;
 ...
 * Name:        unpack_sig
 *
 * Description: Unpack signature sig = (c, LB(z1), len(x), x= Enc(HB(z1)), Enc(h)).
 *
 ...
 **************************************************/
int unpack_sig(uint8_t c_seed[SEEDBYTES], polyvecl *lowbits_z1, polyvecl *highbits_z1,
                polyveck *h, const uint8_t sig[CRYPTO_BYTES]) {
        
    uint8_t encoded_h[MAX_ENC_H_BYTES]; // MAX_ENC_H_BYTES is the upper bound for the size of an encoded h.
    uint8_t encoded_hb_z1[MAX_ENC_HB_Z1_BYTES]; // MAX_ENC_HB_Z1_BYTES is the upper bound for the size of an encoded HB(z1).
    unsigned int i;
    ...
    size_enc_hb_z1 = ((uint16_t) sig[0])<<8 | sig[1]; 
