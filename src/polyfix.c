 ...
 *              - const poly *b: pointer to second summand
 **************************************************/
void polyfix_sub(polyfix *c, const polyfix *a, const poly *b) {
    unsigned int i;
    DBENCH_START();
...
int32_t fix_round(int32_t num){
    num += (num >> 31) & (-LN + 1);
    num += LN / 2; // total \floor(LN / 2) if positive, -\floor(LN / 2) + 1 if negative
    return num / LN;
}
 ...
 *              - const polyveck *v: pointer to second summand
 **************************************************/
void polyfixveck_sub(polyfixveck *w, const polyfixveck *u, const polyveck *v) {
    unsigned int i;
 ...
 *              - const polyvecl *v: pointer to second summand
 **************************************************/
void polyfixvecl_sub(polyfixvecl *w, const polyfixvecl *u, const polyvecl *v) {
    unsigned int i;
 ...
 *
 * Description: Calculates L2 norm of a fixed point polynomial vector with length L + K
 *              The result is L2 norm * LN similar to the way polynomial is usually stored
 *
...
uint16_t polyfixveclk_sample_hyperball(polyfixvecl *y1, polyfixveck *y2, const uint8_t seed[CRHBYTES], const uint16_t nonce)
{
  ...
  do {
    ...
    for (i = 2; i < L + K; i++)
    {
      sample_gauss_N(&samples[N*i], &signs[N/8*i], &sqsum, seed, ni++, N);
    }
    ...
    fp_newton_invsqrt(&invsqrt, &sqsum);
    fp_mul_high(&sqsum, &invsqrt, (uint64_t)(B0 * LN + SQNM/2) << (28-13));

    for (i = 0; i < L; i++)
    {
      for (j = 0; j < N; j++)
      {
        y1->vec[i].coeffs[j] = fp_mul_rnd13(samples[(i*N + j)], &sqsum, (signs[(i*N + j) / 8] >> ((i*N + j) % 8)) & 1);
      }
    }
    for (i = L; i < K + L; i++)
    {
      for (j = 0; j < N; j++)
      {
        y2->vec[i-L].coeffs[j] = fp_mul_rnd13(samples[(i*N + j)], &sqsum, (signs[(i*N + j) / 8] >> ((i*N + j) % 8)) & 1);
      }
    }
