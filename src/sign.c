...
reject:
    ...
    polyveck_add(&b, &b, &s2);
    polyveck_add(&b, &b, &a);
    polyveck_freeze(&b); // TODO necessary?
...
reject:
    ...
    // TODO why do we pack here and not later, after the rejection step?
    ...
    xof256_absorbe_twice(&state, buf, POLYVECK_HIGHBITS_PACKEDBYTES + POLYC_PACKEDBYTES, mu, SEEDBYTES);
    ...
    for (i = 1; i < L; ++i) {
        ...
        invntt_tomont(&cs1.vec[i].coeffs[0]);
    }
    ...
    if (reject1 | reject2) {
        //++counter; counter is already increased
        goto reject;
    }
...
int crypto_sign_verify(const uint8_t *sig, size_t siglen, const uint8_t *m,
    ...
    if (unpack_sig(c_seed, &z1hat, &z1, &h, sig)) {
        return -1;
    }

    // Compose z1 out of HighBits(z1) and LowBits(z1)
    for (i = 0; i < L; ++i) {
        poly_compose(&z1.vec[i], &z1.vec[i], &z1hat.vec[i]);
    }
...
#error "Not yet implemented."
    ...
    z1hat = z1; // TODO avoid this?
    polyvecl_ntt(&z1hat);
    polymatkl_pointwise_montgomery(&highbits, A1, &z1hat);
    ...
    if (polyveclk_sqnorm2(&z1, &z2) > B2SQ) {
        printf("fail: squared norm2 [%lu] > %lu\n", polyveclk_sqnorm2(&z1, &z2),
               B2SQ);
        fflush(stdout);
        return -1;
    }
    ...
    // c_seed = H(HighBits(A * y mod 2q), LSB(round(y0) * j), M)
    xof256_absorbe_twice(&state, buf, POLYVECK_HIGHBITS_PACKEDBYTES + POLYC_PACKEDBYTES, c_seed2, SEEDBYTES);
    xof256_squeeze(c_seed2, SEEDBYTES, &state);
