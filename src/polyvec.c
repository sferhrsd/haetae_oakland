...
void polyveck_poly_fromcrt(polyveck *w, const polyveck *u, const poly *v) {
    unsigned int i, j;
    for (i = 0; i < N; i++) {
        w->vec[0].coeffs[i] = fromcrt(u->vec[0].coeffs[i], v->coeffs[i]);
    }
    for (i = 1; i < K; i++) {
        for (j = 0; j < N; j++) {
            w->vec[i].coeffs[j] = fromcrt0(u->vec[i].coeffs[j]);
        }
    }
}
...
void polyveck_highbits_hint(polyveck *w, const polyveck *v) {
    ...
    for (i = 0; i < K; i++) {
        for (j = 0; j < N; j++) {
            decompose_hint(&w->vec[i].coeffs[j], &dump, v->vec[i].coeffs[j]);
        }
    }
}
...
void polyveck_caddDQ2ALPHA(polyveck *h) {
    ...
    for (i = 0; i < K; i++) {
        for (j = 0; j < N; j++) {
            h->vec[i].coeffs[j] +=
                (h->vec[i].coeffs[j] >> 31) & ((DQ - 2) / ALPHA);
        }
    }
}
...
void polyveck_csubDQ2ALPHA(polyveck *v) {
    ...
    for (i = 0; i < K; i++) {
        for (j = 0; j < N; j++) {
            v->vec[i].coeffs[j] -=
                ~((v->vec[i].coeffs[j] - (DQ - 2) / ALPHA) >> 31) &
                ((DQ - 2) / ALPHA);
        }
    }
}
...
void polyveck_mul_alpha(polyveck *v, const polyveck *u) {
    ...
    for (i = 0; i < K; i++) {
        for (j = 0; j < N; j++) {
            v->vec[i].coeffs[j] = u->vec[i].coeffs[j] * ALPHA;
        }
    }
}
...
void polyveck_round(polyveck *v0, polyveck *v)
{
  for (int i = 0; i < K; i++)
  {
    for (int j = 0; j < N; j++)
    {
      v->vec[i].coeffs[j] = power2round(&v0->vec[i].coeffs[j], v->vec[i].coeffs[j]);
    }
  }
}
 ...
 *norm
 **************************************************/
uint64_t polyveclk_sqnorm2(const polyvecl *a, const polyveck *b) {
    unsigned int i, j;
    uint64_t ret = 0;
...
void polyveck_invntt_tomont(polyveck *x) {
    unsigned int i;
    for (i = 0; i < K; i++) {
        invntt_tomont(&x->vec[i].coeffs[0]);
    }
}
...
static inline void minmaxmask(int32_t *x,int32_t *y,int32_t *mask) // adapted from djbsort
{
...
int64_t polyvecmk_sqsing_value(const polyvecm *s1, const polyveck *s2) {
    ...
    for (size_t i = 0; i < N/TAU+1; i++)
    {
      ...
      fac = (fac&(TAU)) ^ ((~fac)&(N%TAU)); // fac = N%TAU for all != min and TAU for min
