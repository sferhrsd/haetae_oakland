...
void fp_unsigned_signed_mul(fp96_76 *xy, const fp96_76 *y)
{
  fp96_76 x, z;
...
void fp_sub_from_threehalves(fp96_76 *x)
{
  __cneg(x, 1);
...
const fp96_76 start_times_threehalves = {.limb48 = {0x7ad215218533ULL, 0x7ff1c9ULL}};
#elif L == 7
const fp96_76 start_cube              = {.limb48 = {0x700ff3e8890dULL, 0x702ULL}};
const fp96_76 start_times_threehalves = {.limb48 = {0x5768588eed31ULL, 0x73bd40ULL}};
#endif
...
{
  fp96_76 tmp, tmp2;
  fp_mul(&tmp, xhalf, &start_cube); // definitely two positive values
  fp_sub(invsqrtx, &start_times_threehalves, &tmp); // first Newton iteration done, might be negative (very improbable)
  
  for (int i = 0; i < 6; i++) // 6 more iterations
  {
    fp_square(&tmp, invsqrtx); // tmp = y^2, never negative
    fp_mul(&tmp2, xhalf, &tmp); // tmp2 = x/2 * y^2, never negative
    fp_sub_from_threehalves(&tmp2); // tmp = 3/2 - x/2 * y^2
    fp_unsigned_signed_mul(invsqrtx, &tmp2); // y * (3/2 - x/2 * y^2)
  }
}

void fp_init(fp96_76 *x, const uint64_t a[2])
{
  x->limb48[0] = a[0] & ((1ULL<<48)-1);
  x->limb48[1] = (a[0] >> 48) ^ (a[1] << 16);
}
